...
Governments also do not face obstacles to projecting power over space. I estimate $\alpha$ to be `r round(alpha1, 5)`. At this value, if the United States wished to invade China, its effective strength would *increase* due to the distance between the countries. 
...
While governments do not face a geographic loss of strength gradient, wars are still costly. I estimate $\hat{c}$ to be `r round(c_hat, 2)`. Dividing this value by governments' conquest values, shown in Figure \ref{fig:rcv}, I compute war's cost-benefit ratio to be `r round(cb_ratio_mean, 2)` on average. Figure \ref{fig:cb_ratio} divides governments' conquest values by their war cost to produce inverse cost-benefit ratios for each potential war. 

```{r cb_ratio, echo=FALSE, warning=FALSE, message=FALSE, fig.cap=paste0("Estimated inverse cost-benefit ratio for attacking country against each defending country in offensive wars. \\label{fig:cb_ratio}"), fig.height=4, dpi=300, fig.pos="t"}
