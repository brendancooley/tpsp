...
class policies:
    ...
    def theta_bounds(self, bound="lower"):
        ...
        theta_dict_lb = dict()
        theta_dict_lb["eta"] = 1
        theta_dict_lb["gamma"] = alpha_lb
        theta_dict_lb["c_hat"] = c_lb
        theta_dict_lb["alpha1"] = alpha_lb  # distance coefficient
        theta_dict_lb["alpha2"] = alpha_lb  # gdp coefficient
        theta_dict_lb["C"] = np.repeat(c_lb, self.N)
        lb = self.unwrap_theta(theta_dict_lb)

        theta_dict_ub = dict()
        theta_dict_ub["eta"] = 1
        theta_dict_ub["gamma"] = alpha_ub
        theta_dict_ub["c_hat"] = c_ub
        theta_dict_ub["alpha1"] = alpha_ub # distance coefficient
        theta_dict_ub["alpha2"] = alpha_ub  # gdp coefficient
    ...
    def estimator(self, v_sv, theta_x_sv, m, sv=None, nash_eq=False):
        ...
        if nash_eq == False:
            ...
            # problem.set(print_level=5, fixed_variable_treatment='make_parameter', max_iter=self.max_iter_ipopt, linear_solver="pardiso", mu_strategy="adaptive", mu_oracle="probing", fixed_mu_oracle="probing", adaptive_mu_restore_previous_iterate="yes", constr_viol_tol=1.0e-05, dual_inf_tol=1.0, tol=1.0e-05)
            problem.set(print_level=5, fixed_variable_treatment='make_parameter', max_iter=self.max_iter_ipopt, mu_strategy="adaptive", mu_oracle="probing", fixed_mu_oracle="probing", adaptive_mu_restore_previous_iterate="yes", constr_viol_tol=1.0e-05, dual_inf_tol=1.0, tol=1.0e-05)
