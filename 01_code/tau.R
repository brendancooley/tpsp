...
resultsPath <- paste0(projectFiles, "results/", size)
...
rcv_long <- rcv_ft %>% pivot_longer(-j_iso3, names_to="i_iso3", values_to="rcv_ij")
...
data <- tau_long %>% left_join(m_frac_long) %>% left_join(W_long) %>% left_join(Y_j) %>% left_join(Y_i) %>% left_join(X_long) %>% left_join(rcv_long)
