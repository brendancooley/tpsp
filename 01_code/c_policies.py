...
class policies:
    ...
    def estimator(self, v_sv, theta_x_sv, m, sv=None, nash_eq=False):
        ...
        if nash_eq == False:
            ...
            # gamma_theta=1.0e-1
            problem.set(print_level=5, fixed_variable_treatment='make_parameter', max_iter=self.max_iter_ipopt, mu_strategy="adaptive", mu_oracle="probing", fixed_mu_oracle="probing", adaptive_mu_restore_previous_iterate="yes", constr_viol_tol=1.0e-03, dual_inf_tol=1.0e01, tol=1.0e-03, mu_min=self.mu_min, bound_push=.2, theta_min_fact=self.theta_min_fact, max_filter_resets=0, gamma_phi=1.0e-1,required_infeasibility_reduction=1.0e-3)
            # bound_push=.2
