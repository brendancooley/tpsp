...
table_tex <- modelsummary(models_ms, coef_map=cm, add_rows=data.frame(t(fe_row)), gof_omit="AIC|BIC|Log.Lik", title="Inverse Conquest Values and Military Capability Ratios \\label{fig:rcvm_reg_tex}", stars=TRUE, output="latex") %>% kable_styling(latex_options=c("scale_down"))
table_png <- modelsummary(models_ms, coef_map=cm, add_rows=data.frame(t(fe_row)), gof_omit="AIC|BIC|Log.Lik", title="Inverse Conquest Values and Military Capability Ratios", stars=TRUE, output=setup$f_tau_rf_table_path)
